...
export const breezeBaseApi = createApi({
  ...
  endpoints: (builder) => ({
    getProductCategories: builder.query<
      SupaBaseResponse<Array<definitions["product_category"]>>,
      void
    >({
      query: () => `product-category`,
    }),
    getProductsByCategoryName: builder.query<
      SupaBaseResponse<Array<ProductWithRelations>>,
      string
    >({
      query: (categoryName: string) =>
        `product-category/${categoryName}/products`,
    }),
    getProductsForAdminByCategoryName: builder.query<
      SupaBaseResponse<Array<ProductWithRelations>>,
      string
    >({
      query: (categoryName: string) =>
        `product-category/${categoryName}/products?isAdmin=true`,
    }),
    getProductsByCode: builder.query<
      SupaBaseResponse<Array<ProductWithRelations>>,
      ProductInformationProps
    >({
      query: (props: ProductInformationProps) =>
        `products/${props.categoryName}/${props.productCode}`,
    }),
    ...
    getOrderStatus: builder.query<
      SupaBaseResponse<Array<definitions["order_status"]>>,
      void
    >({
      query: () => `orders/order-status`,
    }),
    getOrderItemsByOrderId: builder.query<
      SupaBaseResponse<Array<OrderItemWithRelations>>,
      string
    >({
