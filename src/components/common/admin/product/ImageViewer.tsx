  ...
  return (
    <div className="h-[40vh] overflow-scroll scroll-smooth scrollbar-thin scrollbar-thumb-gray-900 scrollbar-track-gray-100">
      ...
      {productImages.map((image, index) => {
        const imageUrl = image.includes("https://assets")
          ? image
          : `${baseUrl}${image}`;
        const imageText = image.includes("https://assets")
          ? image.split("/images")[1]
          : image.split("assets/")[1];
        return (
          <div
          ...
          >
            <Image
              ...
              alt="Product Image"
            />
            <Text className="w-8/12 overflow-hidden text-sm text-ellipsis">
              {imageText}
            </Text>
            <ActionIcon
