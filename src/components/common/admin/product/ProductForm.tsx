...
const ProductForm: FunctionComponent<ProductFormProps> = (props) => {
  ...
  const [productFormState, setProductFormState] =
    useState<AdminProductFormView>("product-info");
  const [productVariantDeleteState, setProductVariantDeleteState] =
  ...
  const handleImageDelete = async (index: number) => {
    ...
    const { variants, images, ...coreProduct } =
      formatProductFormForUpdate(values);
  ...
  const handleFormCancel = async () => {
    if (values.images) {
      await supabaseClient.storage
        .from("product-images")
        .remove(values.images as string[]);
      handleCancel();
    }
  };
  ...
  return (
    ...
    >
      <section className="flex flex-col">
        ...
        />
        <ImageViewer
          productImages={values.images as string[]}
      ...
      <section className="flex flex-col space-y-4">
        <SegmentedControl
          ...
          onChange={(value) =>
            setProductFormState(value as AdminProductFormView)
          }
          classNames={{
