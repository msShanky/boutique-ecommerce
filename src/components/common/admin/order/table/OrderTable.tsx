...
const OrderTable = ({ data, refreshData }: OrderTableProps) => {
    ...
    const rows = data ? data.map((row) => (
        <tr className='hover:bg-violet hover:text-white hover:opacity-50' key={row.id} onClick={() => orderOnClick(row.id)}>
            {tableHeaders.map(headerData => {
                const { key, span, isDate } = headerData;
                const value = getNestedObjectValue(row, key)
                return (<td colSpan={span}>{isDate ? getDateString(value) : value}</td>)
            })}
        </tr>
    )) : [];
    ...
    const handleOrderEditError = (error: FormErrors, _values: OrderEditFormValues, _event: React.FormEvent<HTMLFormElement>) => {
