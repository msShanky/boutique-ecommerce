...
const baseClass = `flex flex-row space-x-6 items-center text-sm px-3 py-2 w-full hover:cursor-pointer hover:bg-primaryBlack hover:text-white`;

const SidePanelMenuLink: FunctionComponent<SidePanelMenuLinkProps> = (props) => {
	const { isActive, handleMenuClick, item } = props;
	return (
		<a
			className={`${baseClass} ${isActive ? "bg-primaryBlack text-white bg-opacity-80" : "bg-transparent"}`}
