...
const CategoryForm: FunctionComponent<CategoryFormProps> = (props) => {
	...
	const handleImageDelete = async (index: number) => {
		...
		await supabaseClient.storage.from("category-image").remove([imageUrl.split("image/")[1]]);
		const { ...coreCategory } = formatCategoryFormForUpdate(values);
		await supabaseClient.from("product_category").update({ ...coreCategory, category_image: "" });
		...
		<form className="grid grid-cols-2 gap-5" onSubmit={onSubmit((values) => handleSubmit(values))}>
			<section className="flex flex-col">
				...
				{values.category_image && (
					<ImageViewer productImages={[values.category_image as string]} handleImageDelete={handleImageDelete} />
				)}
			</section>

			<CategoryFormDetails categoryForm={categoryForm} />
			<div className="flex justify-end col-start-2 mt-12 space-x-4">
				<Button leftIcon={<IconX />} onClick={handleCancel} variant="filled" className="bg-error" type="reset">
					Cancel
				</Button>
